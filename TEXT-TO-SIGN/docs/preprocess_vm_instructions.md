...
This guides you through setting up a Compute Engine VM and running the end-to-end preprocessing script that replaces the Colab notebook. Visuals are exported to disk instead of shown interactively.
...
gcloud compute instances create preprocessing-vm \
  ...
  --create-disk=auto-delete=yes,name=preprocessing-data-disk,mode=rw,boot=no
```
...
- Output GCS URI (optional upload): e.g. `gs://ghsl-datasets`
...
- Metadata: `/mnt/disks/data/proc/processed_metadata.csv`
...
- If `--output-gcs-uri` is set, the script copies `features/` and `proc/` to that bucket path.

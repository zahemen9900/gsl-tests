...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-gcs-uri", default=None, help="Optional GCS URI to upload outputs (features and proc)")
...
def main():
    ...
    if args.output_gcs_uri:
        run_cmd(["gcloud", "storage", "cp", "-r", output_dir, args.output_gcs_uri.rstrip("/")])
        run_cmd(["gcloud", "storage", "cp", "-r", proc_dir, args.output_gcs_uri.rstrip("/")])
        logging.info("Uploaded outputs to %s", args.output_gcs_uri)
